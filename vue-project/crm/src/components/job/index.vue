<template>
  <div>
    <el-table :data="tableData" style="width: 100%; height: 100%" stripe border>
      <el-table-column type="index" :index="indexMethod" label="编号" width="120" align="center"></el-table-column>
      <el-table-column prop="name" label="职务" width="200" align="center"></el-table-column>
      <el-table-column label="描述" width="200" align="center">
        <template slot-scope="scope">
          <el-popover trigger="hover" placement="top">
            <p>权限: {{ scope.row.desc }}</p>
            <div slot="reference" class="name-wrapper">{{ scope.row.desc }}</div>
          </el-popover>
        </template>
      </el-table-column>
      <el-table-column label="权限" width="420" align="center">
        <template slot-scope="scope">
          <el-popover trigger="hover" placement="top">
            <p>职务: {{ scope.row.name }}</p>
            <p>权限: {{ scope.row.power }}</p>
